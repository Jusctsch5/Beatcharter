...
def check_if_bpm_within_offset(found_bpm: NDArray[float64], correct_bpm: float, offset_bpm: float):
    for bpm in found_bpm:
        logger.debug(f"bpm: {bpm}, correct_bpm: {correct_bpm}, offset_bpm: {offset_bpm}")
        assert bpm > correct_bpm - offset_bpm
        assert bpm < correct_bpm + offset_bpm


def check_bpm_of_file(wrapper: AubioWrapper, file_name, correct_bpm):
    check_if_bpm_within_offset(wrapper.calculate_bpm(file_name), correct_bpm, allowed_bpm_offset)
